...
func (ps *PostRepository) Insert(ctx context.Context, post *entity.Post) error {
	args := ps.Called(ctx, post)
	return args.Error(0)
}

func (ps *PostRepository) FindAll(ctx context.Context, id uuid.UUID, limit, offset int) ([]*entity.Post, error) {
	args := ps.Called(ctx, id)
	return args.Get(0).([]*entity.Post), args.Error(1)
}

func (ps *PostRepository) FindOneByID(ctx context.Context, id uuid.UUID) (*entity.Post, error) {
	args := ps.Called(ctx, id)
	return args.Get(0).(*entity.Post), args.Error(1)
}
...
func (ps *PostRepository) Search(ctx context.Context, query string, limit, offset int) ([]*entity.Post, error) {
	args := ps.Called(ctx, query)
	return args.Get(0).([]*entity.Post), args.Error(1)
}
